...
// The hidden-heart secret — tap the little heart tucked near the cushion to reveal it.
const SECRET_MESSAGE = 'you found Mochi\u2019s secret \u2014 i love you \uD83D\uDC9B';
...
camera.position.set(2.4, 2.0, 6.8);
...
/* ---- Toys & secrets (interactive props on the cushion) ---- */
nose.name = 'nose';
tailSegs.forEach(s => (s.children[0].name = 'tail'));

const yarn = new THREE.Group();
const yarnBall = new THREE.Mesh(new THREE.SphereGeometry(0.26, 24, 20), mat(0xff6b6b, 0.55));
yarnBall.castShadow = true; yarnBall.name = 'yarn'; yarn.add(yarnBall);
for (let i = 0; i < 5; i++) {
  const ring = new THREE.Mesh(new THREE.TorusGeometry(0.263, 0.028, 8, 30), mat(0xe24d4d, 0.5));
  ring.rotation.set(Math.random() * Math.PI, Math.random() * Math.PI, Math.random() * Math.PI);
  yarn.add(ring);
}
yarn.position.set(1.5, 0.34, 1.15);
yarn.userData = { held: false, vy: 0, resting: true };
scene.add(yarn);

const secretHeart = makeHeartMesh(0xff5f8a, 2.2);
secretHeart.name = 'secret';
secretHeart.scale.setScalar(0.16);
secretHeart.position.set(-1.95, 0.42, 1.4);
...
// playful state (moods, gaze, toys, easter eggs)
let excited = 0, cuddle = 0, sleepy = 0, idleTimer = 0, mood = 'content';
let gazeActive = false; const gazePoint = new THREE.Vector3();
let pounceT = 0; const pounceDir = new THREE.Vector3(); let batT = 0, batSide = 0;
let noseTaps = 0, noseTapTimer = 0, sneezeT = 0, tailBurst = 0, secretFound = false;
const treats = [];
const _v1 = new THREE.Vector3(), _v2 = new THREE.Vector3();
let bubbleVisible = false, bubbleTimer = null;
...
function petAt(hit) {
  ...
  cuddle = Math.min(1, cuddle + 0.12); idleTimer = 0;
...
const STAGES = [
  { at: 0,  label: 'new friends', line: 'Meet <b>Mochi</b>. Pet him, drag the yarn, toss a treat — and find his little secret. 🧶' },
...
/* ------------------------------------------------------------------ *
 *  Interaction wiring — pet, yarn, treats, easter eggs
 * ------------------------------------------------------------------ */
const bubbleEl = document.getElementById('bubble');
function showBubble(html, dur = 1400) {
  if (!bubbleEl) return;
  bubbleEl.innerHTML = html; bubbleEl.hidden = false; bubbleVisible = true;
  if (bubbleTimer) clearTimeout(bubbleTimer);
  bubbleTimer = setTimeout(() => { bubbleVisible = false; bubbleEl.hidden = true; }, dur);
}
function onMood(m) {
  const map = { excited: '\uD83D\uDE38 playful!', cuddly: '\uD83D\uDE3B cuddly', sleepy: '\uD83D\uDE34 sleepy\u2026' };
  if (map[m]) showBubble(map[m], 1500);
}

const floorPlane = new THREE.Plane(new THREE.Vector3(0, 1, 0), -0.2);
...
function rayAt(e) {
  ...
  pointer.y = -((e.clientY - r.top) / r.height) * 2 + 1;
  raycaster.setFromCamera(pointer, camera);
  return raycaster.intersectObjects([skull, muzzle, body, nose, yarnBall, secretHeart, ...tailSegs.map(s => s.children[0])], false)[0];
}
...
function beginYarnDrag() {
  yarn.userData.held = true; yarn.userData.resting = false; yarn.userData.vy = 0;
  controls.enabled = false; idleTimer = 0; excited = Math.min(1, excited + 0.3);
  const n = new THREE.Vector3(); camera.getWorldDirection(n);
  dragPlane.setFromNormalAndCoplanarPoint(n, yarn.position);
}
function moveYarnTo(e) {
  const r = canvas.getBoundingClientRect();
  pointer.x = ((e.clientX - r.left) / r.width) * 2 - 1;
  pointer.y = -((e.clientY - r.top) / r.height) * 2 + 1;
  raycaster.setFromCamera(pointer, camera);
  ...
  if (!p) return;
  p.x = clamp(p.x, -2.4, 2.4); p.y = clamp(p.y, 0.34, 2.7); p.z = clamp(p.z, -0.6, 2.2);
  yarn.position.copy(p); excited = Math.min(1, excited + 0.02); idleTimer = 0;
  if (Math.random() < 0.03) spawnHeart(yarn.position.clone().add(new THREE.Vector3(0, 0.3, 0)), { scale: 0.4 });
}

let downX = 0, downY = 0, downType = null;
canvas.addEventListener('pointerdown', (e) => {
  if (phase !== 'bond') return;
  downX = e.clientX; downY = e.clientY; downType = 'floor';
  const hit = rayAt(e);
  if (hit) {
    const n = hit.object.name;
    if (n === 'yarn') { downType = 'yarn'; beginYarnDrag(); pop(640); return; }
    if (n === 'secret') { downType = 'secret'; foundSecret(); return; }
    if (n === 'nose') { downType = 'nose'; noseTap(); return; }
    if (n === 'tail') { downType = 'tail'; tailWiggle(); return; }
    downType = 'pet'; dragging = true; controls.autoRotate = false; petStreak = 0; petAt(hit); pop(600);
  }
});
canvas.addEventListener('pointermove', (e) => {
  if (phase !== 'bond') return;
  if (yarn.userData.held) { moveYarnTo(e); return; }
  if (dragging) { const hit = rayAt(e); if (hit && (hit.object === skull || hit.object === muzzle || hit.object === body)) petAt(hit); }
});
addEventListener('pointerup', (e) => {
  if (yarn.userData.held) {
    yarn.userData.held = false; controls.enabled = true;
    if (yarn.position.distanceTo(cat.position) < 1.7 && yarn.position.y <= 0.5) startPounce(yarn.position);
  } else if (phase === 'bond' && downType === 'floor') {
    if (Math.hypot(e.clientX - downX, e.clientY - downY) < 8) { const fp = floorPoint(e); if (fp) { fp.y = 0.2; tossTreat(fp); } }
  }
  dragging = false; downType = null;
});

document.getElementById('petBtn').addEventListener('click', () => { react(9, true); anim.headDip = 1; cuddle = Math.min(1, cuddle + 0.35); idleTimer = 0; pop(560); });
document.getElementById('feedBtn').addEventListener('click', () => { tossTreat(new THREE.Vector3((Math.random() - 0.5) * 1.4, 0.2, 1.2)); });
document.getElementById('playBtn').addEventListener('click', () => { bounceYarn(); });

/* ---- easter eggs ---- */
function noseTap() {
  noseTaps++; noseTapTimer = 1.2; anim.headDip = 0.6; happyTarget = 1; idleTimer = 0; showBubble('boop!', 700); pop(720);
  if (noseTaps >= 3) { noseTaps = 0; sneeze(); }
}
function sneeze() {
  sneezeT = 0.6; showBubble('achoo! \uD83E\uDD27', 1200); pop(300);
  const p = new THREE.Vector3(); nose.getWorldPosition(p);
  for (let i = 0; i < 5; i++) spawnHeart(p.clone().add(new THREE.Vector3((Math.random() - 0.5) * 0.4, 0.1, 0.2)), { color: 0xffffff, scale: 0.32, ttl: 1.1 });
}
function tailWiggle() {
  tailBurst = 1; happyTarget = 1; idleTimer = 0; excited = Math.min(1, excited + 0.25);
  showBubble('hehe!', 900); pop(660);
  for (let i = 0; i < 3; i++) spawnHeart(cat.position.clone().add(new THREE.Vector3(0, 1.1, -0.9)), { scale: 0.4 });
}
function foundSecret() {
  secretFound = true; idleTimer = 0; showBubble(SECRET_MESSAGE, 3400); pop(880); bumpAffection(4);
  for (let i = 0; i < 14; i++) spawnHeart(secretHeart.position.clone().add(new THREE.Vector3((Math.random() - 0.5) * 0.7, 0.3 + Math.random() * 0.6, (Math.random() - 0.5) * 0.7)), { scale: 0.6, ttl: 2.2 });
}

/* ---- treats + pounce ---- */
function makeTreat() {
  const g = new THREE.Group();
  const b = new THREE.Mesh(new THREE.SphereGeometry(0.15, 18, 14), mat(0x8fc9e6, 0.5)); b.scale.set(1.4, 0.9, 0.8); b.castShadow = true;
  const tf = new THREE.Mesh(new THREE.ConeGeometry(0.12, 0.2, 12), mat(0x8fc9e6, 0.5)); tf.rotation.z = Math.PI / 2; tf.position.x = -0.25;
  g.add(b, tf); return g;
}
function tossTreat(target) {
  const m = makeTreat();
  const from = new THREE.Vector3(cat.position.x + (Math.random() - 0.5) * 0.6, 3.2, cat.position.z + 3.6);
  m.position.copy(from); scene.add(m);
  treats.push({ mesh: m, from, to: target.clone(), t: 0, dur: 0.7 + Math.random() * 0.2, landed: false, eatT: 0 });
  idleTimer = 0; excited = Math.min(1, excited + 0.5); pop(680);
}
function onTreatEaten(pos) {
  bumpAffection(6); happyTarget = 1; cuddle = Math.min(1, cuddle + 0.1); purr(0.4); pop(520);
  for (let i = 0; i < 4; i++) spawnHeart(new THREE.Vector3(pos.x, 0.7, pos.z).add(new THREE.Vector3(0, 0.3, 0)), { scale: 0.5 });
}
function startPounce(target) {
  const d = new THREE.Vector3(target.x - cat.position.x, 0, target.z - cat.position.z);
  pounceDir.copy(d); if (pounceDir.lengthSq() > 1e-4) pounceDir.normalize();
  pounceT = 0.001; anim.headDip = 1; anim.hop = Math.max(anim.hop, 0.5);
  batT = 0.001; batSide = (target.x < cat.position.x) ? 0 : 1;
  excited = Math.min(1, excited + 0.3); idleTimer = 0;
}
function bounceYarn() {
  yarn.visible = true; yarn.userData.held = false; yarn.userData.resting = false; yarn.userData.vy = 3.6;
  yarn.position.set((Math.random() - 0.5) * 1.2, 0.34, 1.2);
  excited = Math.min(1, excited + 0.5); idleTimer = 0; showBubble('\uD83E\uDDF6', 800); pop(700);
}

function updateToys(dt, t) {
  noseTapTimer = Math.max(0, noseTapTimer - dt); if (noseTapTimer === 0) noseTaps = 0;
  sneezeT = Math.max(0, sneezeT - dt);
  tailBurst = Math.max(0, tailBurst - dt * 1.2);
  batT = Math.max(0, batT - dt);

  if (!yarn.userData.held && !yarn.userData.resting) {
    yarn.userData.vy -= 7 * dt; yarn.position.y += yarn.userData.vy * dt;
    if (yarn.position.y <= 0.34) {
      yarn.position.y = 0.34; yarn.userData.vy *= -0.42;
      if (Math.abs(yarn.userData.vy) < 0.7) {
        yarn.userData.vy = 0; yarn.userData.resting = true;
        if (phase === 'bond' && yarn.position.distanceTo(cat.position) < 1.7) startPounce(yarn.position);
      }
    }
  }
  yarn.rotation.y += dt * (yarn.userData.held ? 4 : 1.2);
  yarn.rotation.x += dt * (yarn.userData.held ? 2 : 0.6);

  for (let i = treats.length - 1; i >= 0; i--) {
    const tr = treats[i];
    if (!tr.landed) {
      tr.t += dt / tr.dur; const p = clamp(tr.t, 0, 1);
      tr.mesh.position.x = lerp(tr.from.x, tr.to.x, p);
      tr.mesh.position.z = lerp(tr.from.z, tr.to.z, p);
      tr.mesh.position.y = lerp(tr.from.y, tr.to.y, p) + Math.sin(p * Math.PI) * 1.4;
      tr.mesh.rotation.y += dt * 6;
      if (p >= 1) { tr.landed = true; if (phase === 'bond') startPounce(tr.to); }
    } else {
      tr.eatT += dt;
      if (tr.eatT > 0.3) {
        const k = clamp(1 - (tr.eatT - 0.3) / 0.25, 0, 1); tr.mesh.scale.setScalar(k);
        if (k <= 0) { scene.remove(tr.mesh); treats.splice(i, 1); onTreatEaten(tr.to); }
      }
    }
  }
  ...
  if (phase === 'bond') {
    if (pounceT > 0) {
      pounceT += dt; const e = clamp(pounceT / 0.5, 0, 1); const off = Math.sin(e * Math.PI) * 0.5;
      cat.position.x = pounceDir.x * off; cat.position.z = pounceDir.z * off;
      if (pounceT >= 0.5) pounceT = 0;
    } else {
      cat.position.x = damp(cat.position.x, 0, 6, dt); cat.position.z = damp(cat.position.z, 0, 6, dt);
    }
  }

  secretHeart.rotation.y = t * 1.1;
  secretHeart.position.y = 0.42 + Math.sin(t * 2) * 0.05;
  secretHeart.scale.setScalar((secretFound ? 0.24 : 0.16) * (1 + 0.12 * Math.sin(t * 3)));
  secretHeart.material.emissiveIntensity = 1.8 + Math.sin(t * 3) * 0.5;

  if (bubbleVisible) {
    const v = _v1; parts.head.getWorldPosition(v); v.y += 0.95; v.project(camera);
...
function startJourney() {
  ...
  yarn.visible = false; secretHeart.visible = false; yarn.userData.held = false;
  treats.forEach(tr => scene.remove(tr.mesh)); treats.length = 0;
  pounceT = 0; cat.position.set(0, 0.17, 0);
  bubbleVisible = false; if (bubbleEl) bubbleEl.hidden = true;
...
function tick() {
  ...
  // ----- mood -----
  if (phase === 'bond') {
    excited = Math.max(0, excited - dt * 0.32);
    cuddle = Math.max(0, cuddle - dt * 0.26);
    idleTimer += dt;
    const st = (excited < 0.15 && cuddle < 0.15) ? clamp((idleTimer - 6) / 6, 0, 1) : 0;
    sleepy = damp(sleepy, st, 1, dt);
    const nm = excited > 0.45 ? 'excited' : (cuddle > 0.4 ? 'cuddly' : (sleepy > 0.5 ? 'sleepy' : 'content'));
    if (nm !== mood) { mood = nm; onMood(nm); }
  } else {
    excited = damp(excited, 0, 2, dt); cuddle = damp(cuddle, 0, 2, dt); sleepy = damp(sleepy, 0, 2, dt);
  }

  // ----- gaze target -----
  gazeActive = false;
  if (phase === 'bond') {
    if (yarn.userData.held) { gazePoint.copy(yarn.position); gazeActive = true; }
    else {
      const tr = treats.find(x => !x.landed);
      if (tr) { gazePoint.copy(tr.mesh.position); gazeActive = true; }
      else if (pounceT > 0) { gazePoint.set(cat.position.x + pounceDir.x, 0.5, cat.position.z + pounceDir.z); gazeActive = true; }
      else if (cuddle > 0.3) { gazePoint.copy(camera.position); gazeActive = true; }
      else if (!yarn.userData.resting) { gazePoint.copy(yarn.position); gazeActive = true; }
    }
  }
  let gYaw = 0, gPitch = 0;
  if (gazeActive) {
    const hp = _v1; parts.head.getWorldPosition(hp); const d = _v2.copy(gazePoint).sub(hp);
    gYaw = clamp(Math.atan2(d.x, d.z), -0.7, 0.7);
    gPitch = clamp(-Math.atan2(d.y, Math.hypot(d.x, d.z)), -0.5, 0.5);
  }
  ...
  const bounce = phase === 'bond' ? excited * Math.abs(Math.sin(t * 8)) * 0.05 : 0;
  cat.position.y = 0.17 + (REDUCED ? 0 : Math.sin(t * 1.3) * 0.02) + walkBob + bounce + anim.hop * Math.sin(clamp(1 - anim.hop, 0, 1) * Math.PI) * 0.6;

  const sq = happy * 0.12 + excited * 0.05;
  parts.body.scale.x = 1 + sq; parts.body.scale.z = (1 + sq) * 0.92;

  const dip = Math.max(anim.headDip, anim.feed) * 0.5;
  const sneezeWobble = sneezeT > 0 ? Math.sin((0.6 - sneezeT) * 70) * 0.25 * (sneezeT / 0.6) : 0;
  const baseHeadX = (REDUCED ? 0 : Math.sin(t * 0.8) * 0.05) + dip * 0.4 + walkAmp * Math.sin(t * 6) * 0.04 + sleepy * 0.16 + gPitch;
  parts.head.rotation.x = lerp(parts.head.rotation.x, baseHeadX, 0.2);
  parts.head.rotation.y = lerp(parts.head.rotation.y, gYaw, 0.2);
  parts.head.rotation.z = lerp(parts.head.rotation.z, Math.sin(t * 0.5) * 0.04 + sneezeWobble, 0.3);
  parts.head.position.y = 1.62 - dip * 0.35 + happy * 0.03 - sleepy * 0.06;
  parts.eyeL.position.x = -0.26 + gYaw * 0.05; parts.eyeR.position.x = 0.26 + gYaw * 0.05;
  parts.eyeL.position.y = 0.02 - gPitch * 0.05; parts.eyeR.position.y = 0.02 - gPitch * 0.05;

  const twitch = Math.sin(t * 12) > 0.985 ? 0.2 : 0;
  parts.earL.rotation.z = 0.32 + happy * 0.15 + twitch + excited * 0.14 - sleepy * 0.5;
  parts.earR.rotation.z = -0.32 - happy * 0.15 - twitch - excited * 0.14 + sleepy * 0.5;

  blinkT -= dt; let blink = 1;
  if (blinkT < 0.16) blink = Math.abs(blinkT / 0.08 - 1);
  if (blinkT < 0) blinkT = 2.5 + Math.random() * 3.5;
  let openness = clamp(blink - eyeSquint * 0.55 - happy * 0.25 - sleepy * 0.5 + excited * 0.06, 0.06, 1);
  if (sleepy > 0.4) openness *= (0.55 + 0.45 * Math.abs(Math.sin(t * 1.4)));
  parts.eyeL.scale.y = openness; parts.eyeR.scale.y = openness;

  const stride = t * 9;
  legs.forEach((l, idx) => {
    let r = walkAmp * Math.sin(stride + l.userData.phase) * 0.5;
    if (batT > 0 && idx === batSide) { const be = clamp(batT / 0.4, 0, 1); r += -Math.sin(be * Math.PI) * 1.0; }
    l.rotation.x = r;
  });

  const wag = (0.5 + happy * 1.2 + walkAmp * 0.6 + excited * 1.6 - sleepy * 0.35) * (REDUCED ? 0.4 : 1);
  tailSegs.forEach((s, i) => {
    const amp = (0.18 + i * 0.04) * (1 + happy) * (1 + tailBurst * 2.2);
    s.rotation.z = Math.sin(t * (2 + wag * 2 + tailBurst * 4) - i * 0.5) * amp;
